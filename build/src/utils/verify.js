...
const getScret = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (token && token.length === 2) {
        ...
        const verified = jsonwebtoken_1.verify(matchToken, config_1.default.JWT_SECRET);
        if (verified) {
            if (token) {
                const decoded = jsonwebtoken_1.decode(matchToken);
                console.log(decoded);
                const isAdmin = decoded['isAdmin'];
                let model = isAdmin ? admin_model_1.default : user_model_1.default;
                if (decoded) {
                    try {
                        const adminDetails = yield model.findById(decoded._id);
                        if (adminDetails) {
